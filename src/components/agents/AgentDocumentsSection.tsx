...
import {
  AGENT_DOCUMENT_TYPE_OPTIONS,
  agentDocumentTypeLabel,
  guessAgentDocumentType,
  type AgentDocument,
  type AgentDocumentType,
} from '@/lib/agents/agent-documents-types';
import {
  agentDocumentViewUrl,
  deleteAgentDocument,
  listAgentDocuments,
  uploadAgentDocument,
...
export function AgentDocumentsSection({
  ...
  const [documentType, setDocumentType] = useState<AgentDocumentType>('agency_agreement');
  ...
  const reload = useCallback(async () => {
    ...
    setError('');
    try {
      const docs = await listAgentDocuments(agentMergeKey);
      setDocuments(docs);
    } catch (err) {
  ...
  const resetUploadForm = () => {
    setDocumentType('agency_agreement');
    setSignedDate('');
    setNotes('');
  ...
  const handleFilePick = (file: File | null) => {
    ...
    if (file) {
      setDocumentType(guessAgentDocumentType(file.name));
    }
  };
  ...
  const handleUpload = async () => {
    ...
    setError('');
    try {
      const doc = await uploadAgentDocument({
        agentMergeKey,
        file: selectedFile,
        documentType,
        uploadedBy,
  ...
  return (
    <div className="card" style={{ marginBottom: 16 }}>
      <div className="card-header" style={{ gap: 12 }}>
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 11, color: 'var(--gray)', marginTop: 2 }}>
            Agency agreements, addenda, W-9, and other agent paperwork
          </div>
        </div>
        <button
          type="button"
      ...
      <div className="card-body" style={{ padding: 0 }}>
        {error && (
          <div style={{ padding: '12px 16px', fontSize: 12, color: 'var(--red)', borderBottom: '1px solid var(--gray-border)' }}>
            {error}
          </div>
        )}

        {loading ? (
          <p style={{ padding: 20, fontSize: 13, color: 'var(--gray)' }}>Loading documents…</p>
        ) : documents.length === 0 ? (
          <p style={{ padding: 20, fontSize: 13, color: 'var(--gray)' }}>
            No documents on file for {agentCompany}. Upload an agency agreement or addendum to get started.
          </p>
        ) : (
            ...
            <tbody>
              {documents.map((doc) => (
                <tr key={doc.id}>
                  <td>
                    <a
                      href={agentDocumentViewUrl(doc.id)}
                      target="_blank"
                  ...
                  <td style={{ fontSize: 12 }}>{agentDocumentTypeLabel(doc.documentType)}</td>
                  ...
                  <td style={{ fontSize: 12, color: 'var(--gray)' }}>{doc.uploadedBy || '—'}</td>
          ...
          >
            ...
            <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div>
                ...
                <select
                  value={documentType}
                  onChange={(e) => setDocumentType(e.target.value as AgentDocumentType)}
                  style={inputStyle}
                >
                  {AGENT_DOCUMENT_TYPE_OPTIONS.map((opt) => (
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  placeholder="e.g. Covers 2024–2026 commission terms"
