...
export async function GET(request: Request) {
  ...
  if (!ctx) {
    return NextResponse.json({
      ...
      accounts: [],
      transactions: [],
      summary: { techTotal: 0, txnCount: 0, byCategory: [], techMonthly: 0, techYearly: 0 },
    });
  }
  ...
  type CatAgg = { total90: number; monthly: number };
  ...
  for (const t of techTxns) {
    ...
    const prev = byCategoryMap.get(key) ?? { total90: 0, monthly: 0 };
    prev.total90 += amt;
  ...
  return NextResponse.json({
    ...
    summary: {
      ...
      techMonthly: Math.round(techMonthly * 100) / 100,
      techYearly: Math.round(techMonthly * 12 * 100) / 100,
