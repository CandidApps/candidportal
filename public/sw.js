...
const CACHE = 'candid-pwa-v4';
// Do not precache `/` — it is auth-aware (login vs redirect) and must not be
// served stale after a session is established.
...
self.addEventListener('fetch', (event) => {
  ...
  if (isNavigation) {
    // Network-only for HTML navigations so auth redirects (and session cookies)
    // are never overridden by a cached login page after the PWA is closed.
    event.respondWith(
      fetch(request).catch(
        () =>
          new Response(
            '<!doctype html><title>Candid</title><p>You are offline. Reconnect to open Candid.</p>',
